...
{
    public EOSAutoConnector eosAutoConnector;
    public AudioSource audioPushButton;
    public ParticleSystem particlePushButton;

    public void Start()
    {
        if (eosAutoConnector == null)
        { eosAutoConnector = GameObject.FindObjectOfType<EOSAutoConnector>(); }
    }
    ...
    void OnIncrementalNumberChanged()
    {
        ...
        if (eosAutoConnector)
        { eosAutoConnector.SetupInfoText("The Host button was pressed"); }
    }
